...
interface FunctionRunResultCardProps {
  result: FunctionTestRunResultRow;
  defaultOpen?: boolean;
}

export function FunctionRunResultCard({
  result,
  defaultOpen = false,
}: FunctionRunResultCardProps) {
  ...
  return (
    <div className="rounded-md border">
      ...
      >
        ...
        {result.error ? (
          <Badge variant="destructive" className="px-1.5 py-0 text-[10px]">Error</Badge>
        ) : result.passed ? (
      ...
      {open && (
        <div className="space-y-2 border-t px-3 py-2">
          ...
          {result.expectedOutput != null && (
            <div>
              <p className="text-[10px] font-medium text-muted-foreground">Expected Output</p>
              <pre className="mt-0.5 text-xs bg-muted rounded p-2 overflow-x-auto whitespace-pre-wrap">
          ...
          {result.output != null && (
            <div>
              <p className="text-[10px] font-medium text-muted-foreground">Actual Output</p>
              <pre className={`mt-0.5 text-xs rounded p-2 overflow-x-auto whitespace-pre-wrap ${
