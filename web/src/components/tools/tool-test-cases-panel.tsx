...
import { ToolTestCaseItem } from "./tool-test-case-item";
import { ToolTestCaseCreateForm } from "./tool-test-case-create-form";
...
export function ToolTestCasesPanel({
  ...
  const [runDetailCache, setRunDetailCache] = useState<
    Record<string, ToolTestRunDetail>
  >({});
  ...
  const handleDelete = useCallback(
    async (caseId: string) => {
      await deleteToolTestCase(toolId, caseId, mutate);
    },
    [toolId, mutate]
  );
  ...
  const handleRunAll = useCallback(async () => {
    ...
    const allResults: ToolTestRunResultRow[] = [];
    let passed = 0;

    for (const tc of cases) {
      try {
        const res = await fetch(
          `/api/tools/${toolId}/test-runs/${runId}/case`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
    ...
    try {
      await fetch(
        `/api/tools/${toolId}/test-runs/${runId}`,
        { method: "PATCH" }
      );
    } catch {
      // Non-critical
  ...
  }, [filteredCases, toolId, selectedTags, mutateRuns]);
  ...
  const handleToggleRunDetail = useCallback(
    async (runId: string) => {
      ...
      if (!runDetailCache[runId]) {
        ...
        const detail = await fetchToolTestRunDetail(toolId, runId);
        if (detail) {
    ...
    [expandedRunId, runDetailCache, toolId]
  );

  const handleDeleteRun = useCallback(
    async (runId: string) => {
      setDeletingRunId(runId);
      await deleteToolTestRun(toolId, runId, mutateRuns);
    ...
    [toolId, mutateRuns, expandedRunId]
  );
