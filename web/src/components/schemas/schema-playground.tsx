...
export function SchemaPlayground({ schemaId }: SchemaPlaygroundProps) {
  ...
  const { testCases, mutate: mutateCases } = useSchemaTestCases(schemaId);
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const result = await createSchemaTestCase(
        schemaId,
        {
          name: saveName.trim(),
          input: parsedInput,
          shouldPass: valid === true || valid === null,
  ...
  }, [saveName, inputValue, valid, saveTags, saveShowAsExample, schemaId, mutateCases]);
