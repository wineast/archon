"use client";

import { useCallback } from "react";
import { Badge } from "@/components/ui/badge";
import { cn } from "@/lib/utils";
import type { ObjectTypeRow } from "@/db/schema";

interface ObjectTypeListItemProps {
  objectType: ObjectTypeRow;
  isActive: boolean;
  onSelect: (id: string) => void;
  relationCount: number;
}

export function ObjectTypeListItem({
  objectType,
  isActive,
  onSelect,
  relationCount,
}: ObjectTypeListItemProps) {
  const handleSelect = useCallback(() => {
    onSelect(objectType.id);
  }, [objectType.id, onSelect]);

  return (
    <button
      className={cn(
        "flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-sm hover:bg-accent text-left",
        isActive && "bg-muted font-medium"
      )}
      onClick={handleSelect}
    >
      <span
        className="flex size-5 shrink-0 items-center justify-center rounded text-[10px]"
        style={{ backgroundColor: objectType.color + "20", color: objectType.color }}
      >
        {objectType.icon.charAt(0).toUpperCase()}
      </span>
      <span className="min-w-0 flex-1 truncate">{objectType.name}</span>
      {relationCount > 0 && (
        <Badge variant="secondary" className="shrink-0 text-[10px]">
          {relationCount}r
        </Badge>
      )}
    </button>
  );
}
