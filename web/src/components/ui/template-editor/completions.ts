...
export function createCompletionSource(
  ...
  return (context: CompletionContext): CompletionResult | null => {
    ...
    const options = [
      // Variables (only in {{ }} context or when no specific context)
      ...(!isTagContext
      ...
      // Lookup tables — four variable variants per key (namespaced under lookup.)
      ...(!isTagContext
        ? lookups.flatMap((lk, i) => {
            const detail = lk.name || lk.key;
            const base = 8 - i * 0.01;
            return [
              {
                label: `{{lookup.${lk.key}}}`,
              ...
              {
                label: `{{lookup.${lk.key}_label}}`,
                type: "variable" as const,
                detail: `${detail} (labels)`,
                boost: base - 0.001,
                apply: `{{lookup.${lk.key}_label}}`,
              },
              {
                label: `{{lookup.${lk.key}_json}}`,
                type: "variable" as const,
                detail: `${detail} (JSON)`,
                boost: base - 0.002,
                apply: `{{lookup.${lk.key}_json}}`,
              },
              {
                label: `{{lookup.${lk.key}_entries}}`,
                type: "variable" as const,
                detail: `${detail} (array)`,
                boost: base - 0.003,
                apply: `{{lookup.${lk.key}_entries}}`,
              },
            ];
          })
        : []),
