...
function makeCtx(
  ...
  return { documents, currentDoc: doc, variables, fnExec };
}

function makeExec(fns: Record<string, (input: unknown) => unknown>): FunctionsExec {
  return {
...
describe("processTemplate — function support", () => {
  describe("filter syntax: {{ input | fn_key }}", () => {
    ...
    it("supports string return value", () => {
      ...
      const result = processTemplate(
        "{{ name | greet }}",
        makeCtx(doc, [doc], { name: "World" }, exec)
      );
  ...
  describe("tag syntax: {% fn name input_var %}", () => {
    it("calls function with input variable", () => {
      ...
      const result = processTemplate(
        ...
        makeCtx(doc, [doc], { my_data: { income: 5000, debt: 1500 } }, exec)
      );
  ...
  describe("assign with function: {% assign result = data | fn_key %}", () => {
    it("assigns function result to variable", () => {
      ...
      const result = processTemplate(
        ...
        makeCtx(doc, [doc], { name: "hello" }, exec)
      );
  ...
  describe("backward compatibility", () => {
    it("renders normally without fnExec", () => {
    ...
    it("renders normally with empty exec context (no functions)", () => {
      const exec = makeExec({});
      const doc = makeDoc({ name: "Hello" });
      const result = processTemplate(
        "Title: {{documentTitle}}",
