 ...
 *
 * 来源: GMCC Hermes Rate Sheet 9.15.25 - CA.pdf
 *
 * CA 版本特点：
 * - 贷款金额最高 $4MM
 * - 支持 ADU（附属住宅单元）
 * - FICO 要求相对宽松（$2-3MM 只需 680+）
 */
...
export const HERMES_CA_RATE_SHEET = {
  productName: "Hermes CA",
  ...
  rateSheets: [
    {
      type: "pdf" as const,
      url: "/products/hermes/rate-sheets/GMCC Hermes Rate Sheet 9.15.25 - CA.pdf",
      title: "GMCC Hermes Rate Sheet 9.15.25 - CA",
    },
  ],
} as const;
 ...
 */
export const HERMES_CA_BASE_RATES_7_6_ARM: BaseRate[] = [
 ...
 */
export const HERMES_CA_BASE_RATES_30YR_FIXED: BaseRate[] = [
 ...
 * 所有基准利率
 */
export const HERMES_CA_BASE_RATES: BaseRate[] = [
  ...HERMES_CA_BASE_RATES_7_6_ARM,
  ...HERMES_CA_BASE_RATES_30YR_FIXED,
];
...
const CA_RATE_SHEET_DOC_PATH = "hermes/rate-sheets/GMCC Hermes Rate Sheet 9.15.25 - CA.md";
...
/**
 * Hermes CA FICO × LTV 定价矩阵
 *
 * 按贷款金额分 3 个矩阵，行 = FICO 区间，列 = LTV 区间
 * 引擎自动查表，替代逐条 Adjustment
 */
export const HERMES_CA_LLPA_MATRICES: LLPAMatrix[] = [
  // ≤$2MM
  {
    name: "FICO/LTV (≤$2MM)",
    ...
    citation: {
      ...
      docPath: CA_RATE_SHEET_DOC_PATH,
      lines: { start: 30, end: 31 },
    },
  },
  // $2-3MM（CA 特有，FICO 680+ 即可）
  {
    name: "FICO/LTV ($2-3MM)",
    when: "loanAmount > 2000000 and loanAmount <= 3000000",
    ficoRanges: [
      { label: "680+", min: 680, max: Infinity },
    ],
    ltvRanges: [
      { label: "≤60%", min: 0, max: 60 },
      { label: "60.01-65%", min: 60.01, max: 65 },
    ...
    values: [
      [0.125, 0.125, 0.375, 0.5], // 680+
    ],
    adjustmentType: "rate",
    citation: {
      quote: "| $2MM < Loan Amount ≤ $3MM | 680+ | 0.125% | 0.125% | 0.375% | 0.500% |",
      docPath: CA_RATE_SHEET_DOC_PATH,
      lines: { start: 32, end: 32 },
    },
  },
  // $3-4MM（CA 特有，需 FICO 720+）
  {
    name: "FICO/LTV ($3-4MM)",
    when: "loanAmount > 3000000 and loanAmount <= 4000000",
    ficoRanges: [
      { label: "720+", min: 720, max: Infinity },
    ],
    ltvRanges: [
      { label: "≤60%", min: 0, max: 60 },
      { label: "60.01-65%", min: 60.01, max: 65 },
    ...
    values: [
      [0.125, 0.25, 0.375, null], // 720+（70.01-75% 不可用）
    ],
    adjustmentType: "rate",
    citation: {
      quote: "| $3MM < Loan Amount ≤ $4MM | 720+ | 0.125% | 0.250% | 0.375% | |",
      docPath: CA_RATE_SHEET_DOC_PATH,
      lines: { start: 33, end: 33 },
    },
  },
];
 ...
 * Hermes CA 定价调整项
 *
 * FICO × LTV 矩阵已移至 HERMES_CA_LLPA_MATRICES，此处仅保留单维度调整
 */
export const HERMES_CA_ADJUSTMENTS: Adjustment[] = [
  // ============================================================================
  // Cash-Out 调整（根据 LTV）
  // ============================================================================
  {
  ...
  // ============================================================================
  {
    name: "Units + ADU LTV ≤65%",
    when: "hasADU and ltv <= 65",
    rateAdj: 0.125,
    citations: [
      {
        quote: "| Units + ADU (See ADU guidelines below) | 0.125% | 0.125% | 0.250% | 0.375% |",
        docPath: CA_RATE_SHEET_DOC_PATH,
  ...
  {
    name: "Units + ADU LTV 65.01-70%",
    when: "hasADU and ltv > 65 and ltv <= 70",
    rateAdj: 0.25,
    citations: [
      {
        quote: "| Units + ADU (See ADU guidelines below) | 0.125% | 0.125% | 0.250% | 0.375% |",
  ...
  {
    name: "Units + ADU LTV 70.01-75%",
    when: "hasADU and ltv > 70 and ltv <= 75",
    rateAdj: 0.375,
    citations: [
      {
        quote: "| Units + ADU (See ADU guidelines below) | 0.125% | 0.125% | 0.250% | 0.375% |",
  ...
  {
    name: "ABIO",
    when: 'incomeDocType == "abio"',
    rateAdj: 0.5,
    citations: [
      {
        quote: "| Asset Based Income Option (ABIO) | 0.500% | 0.500% | 0.500% | 0.500% |",
        docPath: CA_RATE_SHEET_DOC_PATH,
  ...
  {
    name: "Bank Statement (3MB/BBS)",
    when: 'incomeDocType == "bankStatement"',
    rateAdj: 0.125,
    citations: [
      {
        quote: "| Banks Statement Options (3MB & BBS) | 0.125% | 0.125% | 0.125% | 0.125% |",
        docPath: CA_RATE_SHEET_DOC_PATH,
 ...
 * 延期费用（price 调整）
 */
export const HERMES_CA_EXTENSION_COSTS: ExtensionCost[] = [
 ...
 */
export const HERMES_CA_REQUIRED_FIELDS = {
  ficoScore: {
    type: "number",
  ...
  loanAmount: {
    type: "number",
    description: "Loan amount in dollars (max $4MM for CA)",
    example: 1500000,
  },
  loanPurpose: {
    type: "enum",
  ...
  hasADU: {
    type: "boolean",
    description: "Whether property has ADU (Accessory Dwelling Unit) - CA only",
    example: false,
  },
  incomeDocType: {
    type: "enum",
 ...
 * Hermes CA 产品完整定价配置
 */
export const HERMES_CA_PRICING_CONFIG = {
  productName: HERMES_CA_RATE_SHEET.productName,
  baseRates: HERMES_CA_BASE_RATES,
  adjustments: HERMES_CA_ADJUSTMENTS,
  llpaMatrices: HERMES_CA_LLPA_MATRICES,
  stackingRules: [],
  extensionCosts: HERMES_CA_EXTENSION_COSTS,
  rateSheets: HERMES_CA_RATE_SHEET.rateSheets,
} as const;
