...
两项架构升级：
1. **Agent Slots**：功能槽位机制，在每个使用点直接选择 Agent，无继承链
...
| `support` | 客服聊天气泡 | support | Build 页面嵌入 |
...
resolveSlot(agentId, slotKey):
  1. 查 agentSlotOverrides(agentId, slotKey) → 有则返回 targetAgentId + modelConfig
...
删除 agent 前检查 `agentSlotOverrides` 是否有引用。有引用则返回 409 Conflict，用户需先解除引用再删除。
...
#### Support Bubble
- 通过 `resolveSlot(agentId, "support")` 获取客服 Agent
- 未配置时不渲染 widget

### SlotAgentSelect 共享组件
...
- `slotKey: SlotKey` - 槽位标识
- `className?: string` - 自定义样式
- `onChanged?: () => void` - 选择变更后回调

紧凑样式 `h-7 text-xs`，支持选择"未配置"清除绑定。

### 组织初始化

`ensureOrgDefaults(orgId)` 创建 4 个默认 agent（builder、assist、evaluator、support），但不自动绑定到任何槽位。用户需在各使用点手动选择。

### API

#### Agent Slots API

- `GET /api/agents/[id]/slots` - 获取 agent 的所有 4 个槽位状态
- `PUT /api/agents/[id]/slots` - 创建/更新槽位绑定（body: `{ slotKey, targetAgentId }`）
- `DELETE /api/agents/[id]/slots` - 删除槽位绑定（body: `{ slotKey }`）

#### Support Bubble API

- `GET /api/agents/[id]/support-bubble` - 获取 support 槽位的 agent + embed token
