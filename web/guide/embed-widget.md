...
气泡按钮支持鼠标和触摸拖拽，松手后自动吸附到最近的视口四角，带平滑动画过渡。

### 交互规则

- **点击 vs 拖拽**：按下后移动距离 < 5px 视为点击（触发开关聊天窗口），≥ 5px 视为拖拽
- **拖拽反馈**：拖拽时气泡有轻微放大（scale 1.05）和更深阴影，cursor 变为 grabbing
- **四角吸附**：松手时气泡自动滑向最近的视口角落（距离边缘 16px），带 0.3s ease-out 动画
- **聊天窗口方向**：吸附到不同角后，聊天窗口弹出方向自动调整（左侧角向右展开，右侧角向左展开，上方角向下展开，下方角向上展开）
- **边界约束**：拖拽时不会超出视口边界
- **窗口 resize**：视口变化导致气泡超出边界时，自动重新吸附到最近的角

### 注意事项

- `data-position` 仅控制初始位置，用户拖拽后位置由运行时决定
- 拖拽开始时会自动关闭已打开的聊天窗口
- 所有现有 API（`open`/`close`/`setContext` 等）不受影响
