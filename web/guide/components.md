...
系统提供 5 个内置 UI 组件，作为 `origin: "builtin"` 的共享池资源存储在数据库中：
...
| ToolCallDefault | `tool-call-default` | 工具调用的默认展示组件，包含可折叠的 PARAMETERS 和 RESULT 区域 |

内置组件在创建新 Agent 时**自动引用**（包括 Slot Agent 和用户创建的 Agent），无需手动添加。其中 `tool-call-default` 是所有工具的通用默认展示——创建新工具时自动关联此组件，对话中未指定自定义组件的工具也会通过 chat 页面的兜底逻辑使用它渲染。

内置组件以**只读**方式展示（不可编辑 JSX/CSS，不可删除），但可以移除引用。
