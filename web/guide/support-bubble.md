...
- 组织初始化时 `ensureOrgDefaults()` 自动创建 support agent 及其默认 embed token
- API 通过 orgSlots 查找 support agent + embed token，无需硬编码 slug

## 数据流

1. 组织初始化：`ensureOrgDefaults(orgId)` 创建 support agent + embed token + orgSlot 记录
...
| `web/src/lib/slots/ensure-org-defaults.ts` | 创建 support agent + embed token |
