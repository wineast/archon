...
/**
 * E2E: Eval concurrency configuration — verifies that the concurrency input
 * in RunEvalDialog works end-to-end: user sets concurrency to 1, runs eval,
 * and the report page displays the correct concurrency value.
 *
 * Flow:
 *   1. Login → Agents page
 *   2. Create test Agent + configure DeepSeek model
 *   3. Create Judge Agent + configure DeepSeek model + Judge Config
 *   4. Test Agent → Eval tab → Create case ("2+2=?" with "contains 4")
 *   5. Run All → Set concurrency to 1 → Confirm
 *   6. Wait for completion → Verify pass rate
 *   7. Report page → Verify concurrency displays "1"
 */

const TIMESTAMP = Date.now()
const TEST_AGENT_NAME = `E2E Concurrency Agent ${TIMESTAMP}`
const JUDGE_AGENT_NAME = `E2E Concurrency Judge ${TIMESTAMP}`

const TAG = "[eval-concurrency]"
...
test.describe("并发数配置 E2E", () => {
  test("concurrency config lifecycle", async ({ page }) => {
    ...
    // ── Step 4: Test Agent → Eval → Create case ──
    await test.step("创建 Eval Case (2+2=?)", async () => {
      ...
      log("创建 eval case: math_concurrency")
      ...
      await caseDialog.locator("input").first().fill("math_concurrency")
      await page.waitForTimeout(300)
    ...
    // ── Step 5: Run All with concurrency = 1 ──
    ...
    await test.step("验证结果：pass rate", async () => {
    ...
    // ── Step 7: Report page → verify concurrency ──
    await test.step("报告页验证并发数显示", async () => {
      log("打开报告页")
      ...
      log("验证并发数显示为 1")
      await expect(newPage.getByTestId("report-concurrency")).toHaveText("1")
      log("报告页 concurrency 验证通过：显示为 1")
