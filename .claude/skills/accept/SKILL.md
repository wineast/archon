...
以下借口**均不成立**，禁止用于跳过 Playwright 验收：

- ❌ "实现者已用 Playwright 自测通过了" → 规约明确说验收者是"独立的第三方裁判"，必须独立验证
- ❌ "typecheck + unit test 已通过，足够了" → 静态检查是必要条件但非充分条件，体验验证必须端到端
- ❌ "改动很小，不影响 UI" → 影响范围由 Playwright 验证确认，不是实现者说了算
- ❌ "逐条标准都看过代码了，不用实际操作" → 规约明确说"必须用 Playwright 实际验证，不能只看 IMPL_REPORT 的 Traceability"
- ❌ "环境起不来" → 先修环境（`make down` + `make up`），不能绕过
- ❌ "时间不够" → 验收是合并门禁，跳过验收等于不验收
