...
1. 调用技能
2. 确认产出文件已生成
3. **E2E 证据审计**（步骤 2/3/4 完成后执行）：
   - 检查对应 `.assets/` 目录是否存在且非空（`ls .task/*.assets/*.png 2>/dev/null`）
   - 解析报告中的 `![](*.png)` 引用，逐一检查引用的文件是否存在
   - 如果截图目录为空或引用的截图文件缺失 → **停止链路**，提示"阶段 N 缺少 E2E 证据（截图），请补充后重跑"
   - 步骤 1（`/requirement`）不做截图审计（需求阶段无 Playwright 要求）
4. 如果产出文件缺失，停止链路并报告失败
5. 继续下一步
