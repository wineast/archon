...
- ❌ "Unit test 已覆盖根因，E2E 多余" → 规约明确说"双层防护 = E2E 守症状 + Unit 守根因，缺一不可"
- ❌ "竞态条件/时序问题难以用 E2E 测试" → Trigger Scenario 必须有 E2E 层级测试，至少覆盖 Happy Path
- ❌ "环境起不来 / Playwright 选择器太难写" → 先用 Playwright MCP 手动走通，再写代码
