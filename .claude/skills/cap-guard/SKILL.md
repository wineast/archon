...
以下借口**均不成立**，禁止用于跳过 E2E 守护测试的编写：

- ❌ "Unit/Integration test 已覆盖标准，E2E 多余" → 规约明确说"水平双层必须有：Criteria Matrix + Journey Test 缺一不可"，Journey Test 必须 E2E
- ❌ "页面选择器太难写 / DOM 结构复杂" → 规约明确说"先用 Playwright MCP 手动走一遍确认选择器，再写测试代码"
- ❌ "竞态条件/时序问题难以用 E2E 测试" → Journey Test 覆盖 Happy Path，不需要复现边缘时序
- ❌ "时间不够，先写 Unit 后补 E2E" → E2E 和 Unit 同时写，不存在"后补"

E2E 测试的**编写**始终是必须的。**执行**（`make e2e`）可在用户同意后跳过（耗时长），但测试代码必须提交。

如确有客观障碍，必须在报告「过程备注」中详细记录障碍原因、已尝试的方法、替代方案，并由用户通过 `AskUserQuestion` 确认接受。
