...
const VERIFY_PATH = join(WORKTREE_DIR, "VERIFY_REPORT.md");
const FIX_PATH = join(WORKTREE_DIR, "FIX_REPORT.md");
...
if (!existsSync(VERIFY_PATH)) {
  console.error("Error: .worktree/VERIFY_REPORT.md not found. Run /verify first.");
  process.exit(1);
}

// --- Determine branch info ---

let baseBranch, currentBranch, isWorktree = false;
let mainRepo = CWD;
let wtName = "";

// Try meta.json for worktree mode
...
// Get current branch
try {
  currentBranch = execSync("git rev-parse --abbrev-ref HEAD", { cwd: CWD, encoding: "utf-8" }).trim();
} catch {}

// Fallback: baseBranch = main
if (!baseBranch) baseBranch = "main";

// Parse merge.sh for MAIN_REPO and WT_NAME (if exists)
let mergeShContent;
if (existsSync(MERGE_SH_PATH)) {
  ...
  if (mainRepoMatch) mainRepo = mainRepoMatch[1];
  if (wtNameMatch) wtName = wtNameMatch[1];
}
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (pathLine && branchLine) {
        const shortBranch = branchLine.replace("branch ", "").replace("refs/heads/", "");
...
const title = wtName || currentBranch || "verify";
...
<div class="container">
  ...
  <div class="chain-flow">
    <span class="chain-node defect">缺陷报告</span>
  ...
  <!-- Tab bar -->
  <div class="tab-bar">
    <button class="tab-btn" data-tab="defect" onclick="switchTab('defect')">
      <span class="tab-badge defect">1</span> 缺陷报告
    </button>
  ...
  <!-- Tab panels -->
  <div class="tab-panel" id="panel-defect">
    <div class="panel-body" id="content-defect"></div>
  </div>
...
<script>
  ...
  // Render all reports
  for (const [key, md] of Object.entries(reports)) {
    if (md) {
      const el = document.getElementById("content-" + key);
      el.innerHTML = rewriteImagePaths(marked.parse(md));
    }
  }

  // Extract verdict from verify report
  const verifyContainer = document.getElementById("content-verify");
  const h2s = verifyContainer.querySelectorAll("h2");
  for (const h2 of h2s) {
    if (h2.textContent.trim().includes("Verdict") || h2.textContent.trim().includes("裁定")) {
      ...
      const verdictHtml = nodes.map(n => n.outerHTML).join("");

      const banner = document.createElement("div");
      banner.innerHTML = verdictHtml;

      if (verdictText.includes("驳回") || verdictText.includes("❌")) {
        banner.className = "verdict-banner verdict-fail";
      } else if (verdictText.includes("有条件") || verdictText.includes("⚠️")) {
...
const server = createServer((req, res) => {
  ...
  // Serve images from .worktree/*.assets/
  if (req.method === "GET" && req.url.startsWith("/assets/")) {
    const relPath = decodeURIComponent(req.url.slice("/assets/".length));
    const filePath = join(WORKTREE_DIR, relPath);

    // Security: prevent path traversal
    if (!filePath.startsWith(WORKTREE_DIR)) {
      res.writeHead(403);
      res.end("Forbidden");
      return;
    }
