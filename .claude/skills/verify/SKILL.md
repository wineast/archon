...
以下借口**均不成立**，禁止用于跳过 Playwright 验证：

- ❌ "修复者已用 Playwright 自测通过了" → 规约明确说"不能只看修复者的截图"，验证者必须独立复现
- ❌ "typecheck + unit test 已通过，足够了" → 静态检查是必要条件但非充分条件
- ❌ "改动很小，不影响 UI" → 影响范围由 Playwright 验证确认，不是修复者说了算
- ❌ "竞态条件/时序问题难以用 E2E 复现" → 至少做基础路径走查 + 边界变体验证
- ❌ "环境起不来" → 先修环境（`make down` + `make up`），不能绕过
- ❌ "时间不够" → 验证是合并门禁，跳过验证等于不验证

如确有客观障碍，必须在报告「过程备注」中详细记录障碍原因、已尝试的方法、替代验证手段，并由用户通过 `AskUserQuestion` 确认接受。
