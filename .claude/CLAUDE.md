...
- 招聘 **FDE（前沿部署工程师）** 分布全国各城市，驻场/远程服务传统企业及已有数字化基础的企业
- FDE 懂业务、不碰代码，在 Archon 上通过对话 + 查看用例效果来配置 Agent，以结果为导向
...
**对企业客户**：痛点不是缺 AI 工具，而是不知道怎么用到自己业务里。FDE 是桥梁——懂业务场景、能梳理需求、用 Archon 快速出效果。比企业自己招 AI 工程师便宜，比找外包公司快。

**对平台方**：中国 to B 市场企业不为"工具"付费，为"解决问题"付费。FDE 人天服务费是进门票，每部署一个 Agent 多一个持续收费节点，每个 Agent 跑起来 token 用量就是持续现金流。**服务拉动平台，平台产生用量**，三层叠加。

**护城河**：Dify 们只有工具没有落地能力，Archon 的模式是**工具 + 人 + 场景**——FDE 网络是分发渠道抄不走，每次部署积累的行业模板和集成 pattern 沉淀回平台让下次更快，母 Agent 越用越聪明。

### 产品推论

当前工具编辑、组件 JSX、LiquidJS 模板等面向开发者的界面是底层能力，后续需要在其上构建一层**对话式 Agent 配置体验**，让 FDE 无需理解底层实现即可完成 Agent 搭建。
...
### E2E 验证（技能链路强制门禁）
- 技能链路（req-chain / defect-chain）中凡是规约标注"必须 Playwright"的阶段，**E2E 是交付门禁，不是可选优化**
- **禁止用任何技术理由跳过 E2E**——"竞态条件难以复现"、"改动很小不影响 UI"、"typecheck + test 已够"等均不成立（详见各技能的"E2E 不可跳过"段落）
- 如确有客观障碍无法执行 E2E，必须在报告中详细记录：障碍原因、已尝试的方法、替代验证手段，并由用户确认接受后方可继续
- **截图资产审计**：报告中引用的每张截图（`![](*.png)`）必须对应真实存在的文件，链路编排器会在阶段间自动验证截图目录非空
